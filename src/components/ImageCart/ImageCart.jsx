...
const ImageCart = ({image,index,handleSelectedImg,handleDragStart,handleDrop,draggedIndex,handleDragOver }) => {
    return (
        ...
        className={
          index === 0
            ? `lg:col-span-2 lg:row-span-2 md:col-span-2 md:row-span-2 sm:col-span-1 sm:row-span-1   border-4 border-gray-300 rounded-md  relative cursor-grab transition-all ${draggedIndex===index?"border-4 border-gray-600":""}`
            : `border-2 border-gray-300 rounded-md  relative cursor-grab transition-all ${draggedIndex===index?"border-2 border-gray-600":""}`
        }
        draggable
      ...
      >
        <img src={image.img} alt="" className={draggedIndex===index?"scale-50":""} />
        {/* hover */}
        <div
          className={
            image.isChecked
